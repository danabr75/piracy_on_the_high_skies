...
module LUIT
  ...
  class LUITElement
    ...
    def initialize(window, holder, id, x, y, z, w, h, color = nil, hover_color = nil, font_color = nil, key_id = nil)
      @window = window
      @holder = holder
      @id = id
      @x = x
      ...
      @color       = color       || 0xff_555555
      @hover_color = hover_color || 0xff_888888
  ...
  class ClickArea < LUITElement
    def initialize(window, holder, id, x, y, z, w = 0, h = 0, color = nil, hover_color = nil, options = {})
      # @window = window
     ...
     # puts "HOVLER COLOR: #{hover_color }"
      ...
      super(window, holder, id, x, y, z, w, h, color, hover_color, options[:font_color], options[:key_id])
    end
  ...
  class TextArea < LUITElement
    attr_reader :field
    def initialize(window, holder, id, x, y, z, maxChar, h)
      h = [10, h].max
      @font = Gosu::Font.new(h)
      w = @font.text_width("W" * maxChar) + 20
      @maxChar = maxChar
      super(window, holder, id, x, y, z, w, h + 20)
      @typing = false
  ...
  #   def initialize(window, holder, id)
  #     @field = Gosu::TextInput.new
  #     # @window = LUIT.window
  #     # @window = window
  #     @holder = holder
  ...
  class ClickPoint < LUITElement
    def initialize(window, holder, id, x, y, z, r)
      @r = [1, r].max
      super(window, holder, id, x - @r, y - @r, z, @r * 2, @r * 2)
    end
  ...
  class Button < LUITElement
    ...
    def initialize(window, holder, id, x, y, z, text, w = 20, h = 50, color = nil, hover_color = nil, font_color = nil, options = {})
     # puts "PARAMS: #{w} - #{h}"
      # h = [50, h].max
      ...
      super(window, holder, id, x, y, z, w, h, color, hover_color, font_color, options[:key_id])
    end
  ...
  #   attr_accessor :value
  #   def initialize(window, holder, id, x, y, z, range)
  ...
  class Toggle < LUITElement
    attr_accessor :value
    def initialize(window, holder, id, x, y, z, size = 30)
      h = [30, size].max
      w = h * 2
      super(window, holder, id, x, y, z, w, h)
      @buttonColor = LUIT.uiColor
      @value = false
    end
