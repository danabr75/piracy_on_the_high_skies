    ...
    def update mouse_x, mouse_y, player_map_pixel_x, player_map_pixel_y, player_x, player_y, player, ships, buildings, options = {}
      ...
      if @time_alive > @last_created_ship_at + @create_ship_every && @ships.count < self.class::SHIP_LIMIT
        ...
        @thread = Thread.new do
          # @creating_ships << AIShip.new(@window, nil, nil, @current_map_tile_x, @current_map_tile_y, {special_target_focus_id: 'player', ship: :pillbug, short_range: true, angle: 0.0, faction_id: self.get_faction_id, no_drops: true})
          @creating_ships << AIShip.new(@window, nil, nil, @current_map_tile_x, @current_map_tile_y, {special_target_focus_id: 'player', angle: 0.0, faction_id: self.get_faction_id, no_drops: true})
