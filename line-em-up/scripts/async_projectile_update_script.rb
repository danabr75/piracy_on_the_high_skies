...
class AsyncProjectileUpdateScript
  ...
  def self.async_update(stdin, stdout, parent_error_prefix)
    debug = false
    # debug = true
...
if ENV["TRIGGER_SUB_PROCESS_BY_PARENT_#{ENV['PARENT_PID']}"] == 'true'
  ...
  file.write("\n GOT IT \n")
  file.flush
  AsyncProjectileUpdateScript.async_update(STDIN, STDOUT, ENV['ERROR_PREFIX'])# if ENV['TRIGGER_PROCESS'] == 'true'
